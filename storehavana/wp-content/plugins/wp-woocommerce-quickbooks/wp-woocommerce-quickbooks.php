...
class vxc_qbooks{            
 public $url='https://www.crmperks.com';           
  ...
  private $temp= '';
  public static $note= '';
  public static $title='WooCommerce QuickBooks Plugin';  
...
*/
  ...
  $message = sprintf(__("%sWooCommerce%s is required. %sDownload latest version!%s", 'wp-woocommerce-quickbooks'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('WooCommerce is installed but not active. %sActivate WooCommerce%s to use the WooCommerce QuickBooks Plugin','wp-woocommerce-quickbooks'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=woocommerce/woocommerce.php'), 'activate-plugin_woocommerce/woocommerce.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sWooCommerce%s is required. %sDownload latest version!%s", 'wp-woocommerce-quickbooks'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");
  }  
...
$res=$this->push($user_id,'save_user');    
}
public function save_product($post_id,$post){
   if(isset($post->post_status) && $post->post_status == 'publish' && !defined( 'DOING_CRON' ) ){ 
...
do_action('vx_addons_save_entry',$id,'','wc','');         
  ...
  */
  ...
  return "<div class='alert_danger crm_alert'>".sprintf(__("Please Configure %sQuickBooks Settings%s",'wp-woocommerce-quickbooks'),"<a href='".$link."'>","</a>")."</div>";
...
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','wp-woocommerce-quickbooks') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  ...
  * This string is sent to email and quickbooks
...
return  $var;
  ...
  */
  ...
  if($note['status'] == "4"){
    $msg=sprintf(__('QuickBooks (%s) Filtered','wp-woocommerce-quickbooks'),$object);    
  }else{
      //web2lead do not have link
  $link=sprintf(__("with ID # %s",'wp-woocommerce-quickbooks'),$this->post('id',$note));
  if($this->post('link',$note) !=""){
      $id_link='<a href="'.$this->post('link',$note).'" target="_blank" title="'.$this->post('id',$note).'">'.$this->post('id',$note).'</a>';
  $link=sprintf(__('with ID # %s','wp-woocommerce-quickbooks'),$id_link);
  }
...
if($note['status'] == '1'){
  $msg=sprintf(__('Added to QuickBooks (%s) ','wp-woocommerce-quickbooks'),$object).$link;
  }else{
  $msg=sprintf(__("Updated to QuickBooks (%s) ",'wp-woocommerce-quickbooks'),$object).$link;     
  }                                         
  }
  }else if($show_error){
     if(empty($error)){
              ...
              $error= __("Error While Posting to QuickBooks",'wp-woocommerce-quickbooks');           
                      }
      }
  $msg=$error;
  }
  ...
  $msg.=' - <a href="'.esc_url($log_url).'" class="vx_log_link" title="'.__('View Detail','wp-woocommerce-quickbooks').'" data-id="'.esc_html($note['log_id']).'">'.__('View Detail','wp-woocommerce-quickbooks')."</a>";
  } 
  return $msg;
  }
  /**
...
///var_dump($items); die();
 ...
 if(method_exists($item,'get_product_id')){
          ...
          $sku=$product->get_sku();
             ...
             $item_info=array(
             __('Title','wp-woocommerce-quickbooks')=>$item->get_name()
             ,__('Quantity','wp-woocommerce-quickbooks')=>$item->get_quantity()
           //  ,__('Line Tax','woo-zoho')=>$item['line_tax']
           //  ,__('Line Subtotal Tax','woo-zoho')=>$item['line_subtotal_tax']
             ,__('Total','wp-woocommerce-quickbooks')=>$item->get_total()
...
return $results;
  ...
  /**
  * Push order to quickbooks feeds
  *     
  * @param mixed $order_id
  * @param mixed $status
  */
...
$object=$this->post('object',$feed); 
 if(!empty($feed['contract_check']) || !empty($feed['customer_check'])){
  if($object == 'Order'){
     $feeds_meta[$e++]=$feed; 
  }else{
  $feeds_meta[$k++]=$feed; 
...
if(!$is_subscription){
  ...
  if($_order){
  self::$order['_order_id']=$_order->get_id();
  } 
}
...
$feed_log=$this->get_feed_log($id,$order_id,$search_object,$parent_id); 
 ...
 if($status == 'restore' && $feed_log['status'] != 5) { // only allow successfully deleted records
  ...
  if(in_array($status, array('delete','restore')) && in_array($feed_log['object'],array('customer'))){ 
  //quickbookes does not support customer deletion
  continue;    
  }
...
//unset($feed['primary_key']);   find new sync token 
...
///var_dump($temp); die();
 if($skip_feed){ continue; }
  ...
  //add account or contract
    ...
    if(!empty($feed['customer_check']) && !empty($feed['object_customer'])){
     $contract_feed=$feed['object_customer']; 
       if( isset(self::$feeds_res[$contract_feed]) ){
   $contract_res=self::$feeds_res[$contract_feed];
  if(!empty($contract_res['id']) && !isset($temp['CustomerRef'])){
   $temp['CustomerRef']=array('value'=> $contract_res['id'],'label'=>'Customer ID');   
  }    
   }
    } 

  //
if(!empty($feed['note_check']) && !empty($feed['note_val']) ){
          $entry_note='';
     $entry_note=$this->process_tags($feed['note_val']);   
          
          if(!empty($entry_note)){
     $temp['Notes']=array('value'=>$entry_note,'label'=>'Feed Note');             
          }
  }
}   
...
$this->send_error_email($order_id,$info,$res);
  ...
  if(!empty($feed['order_notice'])){
   $qres=$res['response']; 
   if(is_array($qres)){
    $qres=reset($qres);
    if(method_exists(self::$_order,'get_total') && isset($qres['TotalAmt'])){
       $wc_total=(int)self::$_order->get_total();
       $qb_total=(int)$qres['TotalAmt']; 
       if($wc_total != $qb_total){
           $email=array('error'=>'WooCommerce Order Total('.$wc_total.') does not match Quickbooks Total('.$qb_total.')');
        $this->send_error_email($order_id,$info,$email);        
       }
    }   
   }   
...
//  if($this->post('disable_log',$settings) !="yes"){
 ...
 // $_order->add_order_note($notice, false); 
  ...
  if(!empty($matches[0])){
   ...
   foreach($matches[0] as $m){
       ...
       if(is_array($val_cust)){ $val_cust=trim(json_encode($val_cust)); }
    $vals['{'.$m.'}']=$val_cust;  
   }
  ...
  */
  ...
  $subject=__('Error While Posting to QuickBooks','wp-woocommerce-quickbooks');
$page_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; 
  ...
  $email_info=array("msg"=>$res['error'],"title"=>__("QuickBooks Error",'wp-woocommerce-quickbooks'),"info_title"=>"More Detail","info"=>array("Order Id"=>$order_url,"Time"=>date('d/M/y H:i:s',current_time('timestamp')),"Page URL"=>'<a href="'.esc_url($page_url).'" style="word-break:break-all;">'.esc_url($page_url).'</a>'));
  ...
  */
  ...
  $status=0;
  if(class_exists('vxc_qbookss_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/wp-woocommerce-quickbooks/woocommerce-qbooks-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
$objects=array('customer'=>'Customer','invoice'=>'Invoice');
$objects=apply_filters('quickbooks_woo_objects_list',$objects);  
  return $objects;    
 }
...
$vxc_qbooks=new vxc_qbooks(); 
$vxc_qbooks->instance();
$vx_wc['vxc_qbooks']='vxc_qbooks';
